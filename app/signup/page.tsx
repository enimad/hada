...
export default function SignupEntryPage() {
  ...
  function submitEmail() {
    startTransition(async () => {
      ...
      try {
        response = await fetch("/api/auth/check-email", {
          method: "POST",
      ...
      const result = (await response.json().catch(() => ({ error: "Réponse serveur illisible." }))) as { exists?: boolean; error?: string };
      if (!response.ok) {
  ...
  return (
    <MobileScreen className="min-h-[100svh] justify-start overflow-y-auto pb-[clamp(24px,5svh,64px)] pt-[clamp(40px,7svh,90px)]">
      <div className="shrink-0 text-center">
        <Link href="/" aria-label="Retour à l'accueil Hada" className="inline-block">
          <HadaWordmark className="mx-auto !w-[clamp(158px,17svh,216px)] !max-w-[66vw]" />
        </Link>
        <p className="mx-auto mt-[clamp(14px,2.2svh,24px)] max-w-[270px] text-[clamp(16px,2.2svh,22px)] font-semibold tracking-[-0.035em] text-[var(--hada-navy)] sm:max-w-none">
          Ton wedding planner de poche
        </p>
      </div>

      <div className="w-full pt-[clamp(48px,6.4svh,72px)]">
        <div>
        ...
        <div className="mt-[clamp(38px,5.2svh,56px)]">
          <button
            type="button"
        ...
        <DividerOr className="my-[clamp(28px,4svh,54px)] text-[clamp(14px,1.9svh,17px)]" />
      ...
      <div className="mt-auto flex shrink-0 flex-wrap items-center justify-center gap-x-4 gap-y-1.5 pt-[clamp(42px,8svh,110px)] text-center text-[clamp(11px,1.6svh,13px)] font-medium text-[#8d8387]">
